import React, { memo, useState, useMemo, useReducer, useCallback } from "react";
...
const Option = memo(function Option(props) {
  ...
  //   (value) => {
  //     const newCheckdMap = { ...checkedMap };
  //     if (value in checkedMap) {
  //       delete newCheckdMap[value];
  //     } else {
  //       newCheckdMap[value] = true;
  //       update(newCheckdMap);
  //     }
  //   },
  //   [checkedMap, update]
...
const BottomModal = memo(function BottomModal(props) {
  ...
  // state的初始值在组件第一次渲染的时候才会被用到，所以在每个生命周期都重新声明一下就太浪费性能
  // 所以用到延迟初始化state
  ...
  const optionGroup = [
    {
      title: "坐席类型",
      options: ticketTypes,
      checkedMap: localCheckedTicketTypes, // 置换成本地的版本
